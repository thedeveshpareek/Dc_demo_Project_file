...
function AgentEditForm(props:any) {
    const {agent, onUpdate,onCancel,...properties} = props;
    ...
    const handleSubmit = (e: any) => {
        e.preventDefault();
        setIsSaving(true);
        AgentService.save(values).then(response => {
            setIsSaving(false);
            setIsSaved(true);
            setIsError(false);
            ...
            <div className="form-body">
                ...
                <div className="row">
                    {FORM.AGENT.map((input) => (
                        <FormField key={input.id} {...input} onChange={onChange} defaultValue={agent[input.name]} />
                    ))}
                </div>
            </div>
