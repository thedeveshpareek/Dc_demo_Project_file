...
import TransferEditForm from "../../../components/edit-forms/TransferEditForm";

function AccountTransferList(props: any) {
    const navigate = useNavigate();
    const permission = AuthService.getPermission('ACCOUNT_TRANSFER');
    ...
    const columns: {}[] = [
        ...
        {data: "fromAccountName", name: "Account From", class: ''},
        {data: "toAccountName", name: "Account To", class: ''},
        {data: "amount", name: "Amount", class: "text-right", currency: true},
        {data: "description", name: "Description"},
        {name: "Action", render: rowActions, class: "text-center p-0"}
    ];
    const onEdit = (e: any) => {
        setTransfer(JSON.parse(e.target.dataset.record));
        setAction('edit');
    }
    const onAdd = (e: any) => {
        setAction('add');
    ...
    const handleSearch = (params: any) => {
        AccountTransferService.findAllByAllColumn(params).then(response => {
            setDate(response.data);
        }).catch(reason => {
            if (reason.code === "ERR_NETWORK") {
                navigate("/maintenance");
                ...
                <div className="content-wrapper">
                    ...
                    <div className="content-header row">
                        <div className="content-header-left col-md-4 col-12 mb-2">
                            <h3 className="content-header-title">Account Transfer List</h3>
                        </div>
                            ...
                            <div className="breadcrumbs-top float-md-right">
                                <div className="breadcrumb-wrapper mr-1">
                                    <ol className="breadcrumb">
                                        ...
                                        <li className="breadcrumb-item active">Transfers
                                        </li>
                                    </ol>
                                </div>
                            </div>
                            ...
                            <div className="card-content collapse show">
                                <div className="card-header">
                                    {permission.create?<button type={'button'} className="btn btn-sm btn-info box-shadow-1 pull-right"
                                            onClick={onAdd}>
                                        <i className="ft-plus"></i>Add Transfer
                                    </button>:''}
                                </div>
                                <div className="card-body">
                                    <DataTable columns={columns} data={data} onSearch={handleSearch}
                                               endpoint={API_ROUTES.ACCOUNT_TRANSFER_ADVANCE_SEARCH} refresh={refresh}
                        ...
                        <div className="modal-dialog modal-lg modal-dialog-centered" role="document">
                            <div className="modal-content scroll-95">
                                <div className="modal-body">
                                    <TransferForm onSuccess={onUpdate} onCancel={onCancel}/>
                                </div>
                            </div>
                        </div>
                    </div> : ""
                ...
                action == 'edit' ?
                    <div className={`modal fade fadeIn show`} role="dialog"
                        ...
                        <div className="modal-dialog modal-lg modal-dialog-centered" role="document">
                            <div className="modal-content scroll-95">
                                <div className="modal-body">
                                    <TransferEditForm transfer={transfer} onSuccess={onUpdate} onCancel={onCancel}/>
                                </div>
                            </div>
                        </div>
                    </div> : ""
