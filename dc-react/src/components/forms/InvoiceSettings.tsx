...
function InvoiceSettingsForm(props: any) {
    ...
    const handleSubmit = (e: any) => {
        ...
        ReportSettingsService.saveInvoice(formData).then(response => {
            setIsSaved(true);
            setIsError(false);
    ...
    return (
        <form className="form" onSubmit={handleSubmit} encType={"multipart/form-data"}>
            ...
            <div className="form-body mt-3">
                ...
                <div className="row">
                    {FORM.INVOICE_SETTINGS.map((input) => (
                        <FormField key={input.id} {...input} onChange={onChange} defaultValue={values[input.name]} accept={'.jpg,.png,.webp,.jpeg,.pdf'}/>
                    ))}
                </div>
            </div>
