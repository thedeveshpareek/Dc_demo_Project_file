...
function BusinessSettingsForm(props: any) {
    ...
    const handleSubmit = (e: any) => {
        ...
        BusinessSettingsService.save(formData).then(response => {
            setIsSaved(true);
            setIsError(false);
            setMessage("Business Settings Successfully Saved!");
            e.target.reset();
            setTimeout(args => {
                setIsSaved(false);
                setIsSaving(false);
            ...
            if (reason.response.status == 401) {
                ...
                setMessage("Sorry! Tou don't have permission to save business settings");
            } else {
    ...
    const loadCurrentValues = ()=>{
        BusinessSettingsService.find().then(response => {
        ...
        <form className="form" onSubmit={handleSubmit} encType={"multipart/form-data"}>
            ...
            <div className="form-body mt-3">
                <div style={{display: "flex", justifyContent: "flex-start"}} className="form-section">
                    <h3>Business Settings Info</h3>
                </div>
                <div className="row">
                    {FORM.BUSINESS_SETTINGS.map((input) => (
                ...
                <div className="row">
                    <div className="col-4">
                        ...
                        <input type="file" name={'logo'} className={'form-control'} multiple={false} accept={'.jpg,.png,.webp,.jpeg,.pdf'}  onChange={onChange}/>
                    </div>
                </div>
            </div>
