...
function AgencyEditForm(props:any) {
    const {agency, onUpdate,onCancel,...properties} = props;
    ...
    const handleSubmit = (e: any) => {
        e.preventDefault();
        setIsSaving(true);
        AgencyService.save(values).then(response => {
            setIsSaving(false);
            setIsSaved(true);
            setIsError(false);
            setMessage("Agency Successfully Updated!");
            onUpdate(response)
        }).catch(reason => {
            setIsSaving(false);
            ...
            <div className="form-body">
                ...
                <div className="row">
                    {FORM.AGENCY.map((input) => (
                        <FormField key={input.id} {...input} onChange={onChange} defaultValue={agency[input.name]} />
                    ))}
                </div>
            </div>
