...
import AgentEditForm from "../../../components/edit-forms/AgentEditForm";

function AgentList(props: any) {
    const navigate = useNavigate();
    const permission = AuthService.getPermission('AGENT');
    ...
    const columns: {}[] = [
        ...
        {data: "commissionAmount", name: "Commission Amount", class: "text-right ", calculateSum: true, currency: true},
        {name: "Action", render: rowActions, class: "text-center"}
    ];
    const onEdit = (e: any) => {
        setAgent(JSON.parse(e.target.dataset.record));
        setOnEditing(true);
    }
    const onView = (e: any) => {
    ...
    const loadData = () => {
        AgentService.findAll().then(response => {
    ...
    const handleSearch = (params: any) => {
        AgentService.findAllByAllColumn(params).then(response => {
            setDate(response.data);
        }).catch(reason => {
            if (reason.code === "ERR_NETWORK") {
                navigate("/maintenance");
                ...
                <div className="content-wrapper">
                    ...
                    <div className="content-header row">
                        <div className="content-header-left col-md-4 col-12 mb-2">
                            <h3 className="content-header-title">Agent List</h3>
                        </div>
                            ...
                            <div className="breadcrumbs-top float-md-right">
                                <div className="breadcrumb-wrapper mr-1">
                                    <ol className="breadcrumb">
                                        ...
                                        <li className="breadcrumb-item"><a href="/agents">Agent</a>
                                        </li>
                        ...
                        <div className="card">
                            <div className="card-content collapse show">
                                <div className="card-header">
                                    {permission.create?<Link to="/agents/create" className="btn btn-sm btn-info box-shadow-1 pull-right"><i
                                        className="ft-plus"></i> Add Agent</Link>:''}
                                </div>
                                <div className="card-body">
                                    <DataTable columns={columns} data={data} onSearch={handleSearch}
                                               endpoint={API_ROUTES.AGENT_ADVANCE_SEARCH} refresh={refresh}
                                               searchPlaceholder={"Search Agent By FullName"}/>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="modal-dialog modal-lg modal-dialog-centered" role="document">
                            <div className="modal-content scroll-95">
                                <div className="modal-body">
                                    <AgentEditForm agent={agent} onUpdate={onUpdate} onCancel={onCancel}/>
                                </div>
                            </div>
                        </div>
                    </div>: ""
